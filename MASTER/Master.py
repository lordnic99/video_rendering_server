...
def do_work(connection, channel, delivery_tag, body):
    ...
    Common.log_message(f"Đã nhận được job mới: {job_name}")
    Common.log_message(f"Tiến hành chuẩn bị cho job mới")

    if job_name in Common.get_job_result():
        Common.log_message("Job đã tồn tại, tiến hành bỏ qua")
        cb = functools.partial(ack_message, channel, delivery_tag)
        connection.add_callback_threadsafe(cb)
        return
    ...
    subprocess.run(
        ["python", "RenderService.py"] + [job_name], check=True, text=True, shell=True
    )
