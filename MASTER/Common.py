...
def jpg_copy(file_path):
    ...
    destination_folder = os.path.normpath(os.path.join(os.path.dirname(__file__), r'..\RenderService\input\video'))
...
def audio_copy(file_path):
    if file_path.endswith(".m4b"):
        ...
        convert_m4b_to_mp3(file_path, os.path.join(os.path.normpath(os.path.join(os.path.dirname(__file__), r'..\RenderService\input\audio')), str(file_path).rsplit(os.sep, 1)[-1]).split(".")[0] + ".mp3")
        return
    file_name = os.path.basename(file_path)
    
    destination_folder = os.path.normpath(os.path.join(os.path.dirname(__file__), r'..\RenderService\input\audio'))
...
def convert_m4b_to_mp3(input_file, output_file):
    ...
    result = subprocess.run([ffmpeg_path, '-i', input_file, '-acodec', 'libmp3lame', output_file], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    return result.returncode
        
def prepare_job_before_receive():
    try:
        current_directory = os.getcwd()
        audio_input_directory = os.path.join(current_directory, "../RenderService/input/audio")
        video_input_directory = os.path.join(current_directory, "../RenderService/input/video")
        temp_input_directory = os.path.join(current_directory, "../RenderService/input/audio/temp")
        ...
        for directory in [audio_input_directory, video_input_directory, temp_input_directory, video_tmp_mp4, output_dir]:
...
def get_audio_duration_hours(file_path):
    try:
        ...
        duration_hours = duration_seconds / 3600  # Convert duration from seconds to hours
...
def split_audio(input_file, output_pattern, segment_duration_minutes):
    try:
        ...
        subprocess.run([ffmpeg_path, '-i', input_file, '-f', 'segment', '-segment_time', str(segment_duration_seconds), '-c', 'copy', output_pattern], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def join_all_audio(audio_list, output_file):
    ...
    for m4b_file in m4b_list:
        ...
        mp3_file = os.path.normpath(os.path.join(os.path.dirname(__file__), r'..\RenderService\input\audio')) + "\\" + os.path.splitext(os.path.basename(m4b_file))[0] + ".mp3"
    ...
    os.chdir(os.path.normpath(os.path.join(os.path.dirname(__file__), r'..\RenderService')))
    command = [
        "./bin/ffmpeg",
        "-f", "concat",
        "-safe", "0",
